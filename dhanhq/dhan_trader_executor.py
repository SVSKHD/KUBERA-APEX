...
def place_order_dhan(dhan, symbol, action, current_price, stop_loss, take_profit, lot_size, is_option=False,
                     strike_price=None, option_type=None, expiry_date=None):
    order_type = 'BUY' if action == 'BUY' else 'SELL'
    if is_option:
        # Placeholder function, replace with actual order placement API call for options
        print(
            f"Placing {order_type} option order for {symbol} (Strike: {strike_price}, Type: {option_type}, Expiry: {expiry_date}) at {current_price} with SL {stop_loss} and TP {take_profit}")
    else:
...
def update_trailing_stop_loss(active_trades, current_price):
    for trade in active_trades:
        if trade["action"] == "BUY":
            if current_price > trade["highest_price"]:
                trade["highest_price"] = current_price
                trade["stop_loss"] = current_price - trade["trail_amount"]
